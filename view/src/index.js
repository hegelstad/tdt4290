import React from "react";
import Autosuggest from 'react-autosuggest'

const GremlinView = () => {
    const labels = [
        {
            "name": "Swagger Schema",
            "count": 1034
        },
        {
            "name": "Computer",
            "count": 811
        },
        {
            "name": "Javascript",
            "count": 788
        },
        {
            "name": "Typescript",
            "count": 442
        },
        {
            "name": "Swagger Parameter",
            "count": 325
        },
        {
            "name": "Business Capability",
            "count": 293
        },
        {
            "name": "Directory",
            "count": 284
        },
        {
            "name": "Advisory, Product, Suitability, Disclosure and Fiduciary Issues",
            "count": 264
        },
        {
            "name": "Control",
            "count": 255
        },
        {
            "name": "Swagger Response",
            "count": 250
        },
        {
            "name": "Improper Business or Market Practices",
            "count": 243
        },
        {
            "name": "Transaction Capture, Execution and Maintenance",
            "count": 237
        },
        {
            "name": "Application",
            "count": 146
        },
        {
            "name": "Technical Capability",
            "count": 127
        },
        {
            "name": "Swagger Structure",
            "count": 107
        },
        {
            "name": "Field",
            "count": 105
        },
        {
            "name": "Swagger Operation",
            "count": 95
        },
        {
            "name": "Template",
            "count": 94
        },
        {
            "name": "Person",
            "count": 87
        },
        {
            "name": "Contract",
            "count": 71
        },
        {
            "name": "Organization",
            "count": 68
        },
        {
            "name": "Unauthorised Activity",
            "count": 59
        },
        {
            "name": "Code Library",
            "count": 54
        },
        {
            "name": "Swagger Path",
            "count": 53
        },
        {
            "name": "Vulnerability",
            "count": 51
        },
        {
            "name": "External Fraud",
            "count": 41
        },
        {
            "name": "Entity",
            "count": 36
        },
        {
            "name": "Department",
            "count": 36
        },
        {
            "name": "Actor",
            "count": 36
        },
        {
            "name": "Strategic Risks",
            "count": 36
        },
        {
            "name": "Internal Fraud",
            "count": 36
        },
        {
            "name": "Style",
            "count": 35
        },
        {
            "name": "Operational Risks",
            "count": 33
        },
        {
            "name": "Programming Language",
            "count": 31
        },
        {
            "name": "Observation",
            "count": 30
        },
        {
            "name": "General HR Disputes",
            "count": 29
        },
        {
            "name": "Printer",
            "count": 28
        },
        {
            "name": "Accounting Errors",
            "count": 26
        },
        {
            "name": "Idea",
            "count": 23
        },
        {
            "name": "Documentation Errors",
            "count": 21
        },
        {
            "name": "Component Type",
            "count": 21
        },
        {
            "name": "Project",
            "count": 20
        },
        {
            "name": "Server",
            "count": 19
        },
        {
            "name": "Compliance Risks",
            "count": 18
        },
        {
            "name": "Domain",
            "count": 15
        },
        {
            "name": "Consumable",
            "count": 15
        },
        {
            "name": "Metric",
            "count": 15
        },
        {
            "name": "Step",
            "count": 14
        },
        {
            "name": "Product / Service",
            "count": 14
        },
        {
            "name": "Clause",
            "count": 14
        },
        {
            "name": "Value Stream Component",
            "count": 13
        },
        {
            "name": "Graphic",
            "count": 11
        },
        {
            "name": "Software",
            "count": 11
        },
        {
            "name": "Model Design, Pricing and Testing Errors",
            "count": 10
        },
        {
            "name": "Operating System",
            "count": 10
        },
        {
            "name": "Value Proposition",
            "count": 10
        },
        {
            "name": "Container",
            "count": 9
        },
        {
            "name": "Channel",
            "count": 9
        },
        {
            "name": "Windows Server",
            "count": 8
        },
        {
            "name": "Font",
            "count": 8
        },
        {
            "name": "Compensation Process Issues",
            "count": 8
        },
        {
            "name": "UNIX Server",
            "count": 7
        },
        {
            "name": "Empathy Map Component",
            "count": 7
        },
        {
            "name": "Financial Reporting Risks",
            "count": 7
        },
        {
            "name": "Business Disruption and System Failures",
            "count": 7
        },
        {
            "name": "Business Outcome",
            "count": 6
        },
        {
            "name": "Key Partner",
            "count": 6
        },
        {
            "name": "Subject Area",
            "count": 5
        },
        {
            "name": "Network Gear",
            "count": 5
        },
        {
            "name": "Rack",
            "count": 5
        },
        {
            "name": "Manual Step",
            "count": 4
        },
        {
            "name": "Process",
            "count": 4
        },
        {
            "name": "Linux Server",
            "count": 4
        },
        {
            "name": "Key Activity",
            "count": 4
        },
        {
            "name": "Revenue Stream",
            "count": 4
        },
        {
            "name": "Persona",
            "count": 4
        },
        {
            "name": "Network zone",
            "count": 4
        },
        {
            "name": "Technology Group",
            "count": 4
        },
        {
            "name": "Cost Structure",
            "count": 4
        },
        {
            "name": "Theme",
            "count": 4
        },
        {
            "name": "Key Resource",
            "count": 4
        },
        {
            "name": "Storage Device",
            "count": 3
        },
        {
            "name": "Customer Relationship",
            "count": 3
        },
        {
            "name": "Automated Step",
            "count": 2
        },
        {
            "name": "Web Server",
            "count": 2
        },
        {
            "name": "Customer Segment",
            "count": 2
        },
        {
            "name": "Hidden Container",
            "count": 2
        },
        {
            "name": "AIX Server",
            "count": 2
        },
        {
            "name": "Computer Peripheral",
            "count": 2
        },
        {
            "name": "Persona Map",
            "count": 1
        },
        {
            "name": "Journey",
            "count": 1
        },
        {
            "name": "Swagger Specification",
            "count": 1
        }
    ];
      
      // https://developer.mozilla.org/en/docs/Web/JavaScript/Guide/Regular_Expressions#Using_Special_Characters
      function escapeRegexCharacters(str) {
        return str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      }
      
      function getSuggestions(value) {
        const inputValue = value.trim().toLowerCase();
        const inputLength = inputValue.length;

        return inputLength === 0 ? [] : labels.filter(label => label.name.toLowerCase().slice(0, inputLength) === inputValue
  );
      }
      
      function getSuggestionValue(suggestion) {
        return suggestion.name;
      }
      
      function renderSuggestion(suggestion) {
        return (
          <span>{suggestion.name}</span>
        );
      }
      
      class App extends React.Component {
        constructor() {
          super();
      
          this.state = {
            value: '',
            suggestions: []
          };    
        }
      
        onChange = (event, { newValue, method }) => {
          this.setState({
            value: newValue
          });
        };
        
        onSuggestionsFetchRequested = ({ value }) => {
          this.setState({
            suggestions: getSuggestions(value)
          });
        };
      
        onSuggestionsClearRequested = () => {
          this.setState({
            suggestions: []
          });
        };
      
        render() {
          const { value, suggestions } = this.state;
          const inputProps = {
            placeholder: "Type 'c'",
            value,
            onChange: this.onChange
          };
      
          return (
            <Autosuggest 
              suggestions={suggestions}
              onSuggestionsFetchRequested={this.onSuggestionsFetchRequested}
              onSuggestionsClearRequested={this.onSuggestionsClearRequested}
              getSuggestionValue={getSuggestionValue}
              renderSuggestion={renderSuggestion}
              inputProps={inputProps} />
          );
        }
      }
      
      ReactDOM.render(<App />, document.getElementById('app'));
      
    }


export default GremlinView;
